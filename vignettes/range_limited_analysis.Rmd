...
vignette: >
  %\VignetteIndexEntry{range_limited_analysis}
...
nearest road bridge. We can explore this phenomenon using *PlasmoMAPI* by
...
barrier_list <- list(data.frame(long = c(-5,0,6,7,4,-3,-2,3,4,3,-5),
                                lat = c(5,7,6,0,-8,-4,-3,-4,-1,4,4)))
barrier_list[[1]] <- rbind(barrier_list[[1]], barrier_list[[1]][1,])
...
exist between these locations. We will use this to generate genetic distances,
this time on the [-Inf,Inf] interval rather than the [0,1] interval just for a
change.
...
p <- load_data(p, genetic_distance, check_delete_output = FALSE)
...
entire range of data, and a second time restricting to distances less than
500km:
...
However, throwing data away like this comes at a cost - notice there are now
quite a number of hexes that are below our required coverage level, and so are
shown in grey. The more we subset the range the fewer edges we we have at our
